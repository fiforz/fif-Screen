...
- 同一局域网内使用相同四位 PIN 自动发现并双向认证，广播受限时可手动输入电脑 IPv4；
...
2. 选择“USB 调试”，点击“启动扩展屏”。控制中心会等待虚拟显示器就绪并自动切换到扩展桌面；首次启动还会把安装包内的 APK 安装到已授权的 Android 设备。
...
2. 在 Windows 控制中心选择“无线局域网”，输入任意四位数字 PIN，点击“应用 PIN 并等待连接”。控制中心会显示电脑的 IPv4 地址。
3. 打开 Android 应用，按系统返回键或点击右上角设置按钮，选择“无线局域网连接”。
4. 输入与 Windows 控制中心相同的四位 PIN，保留“自动发现”并点击“连接”。应用会在 Wi-Fi/以太网接口上寻找电脑并建立画面与触控通道。
5. 自动发现失败时，选择“手动电脑 IP”，输入 Windows 控制中心显示的 IPv4 地址后重新连接。
6. PIN 不通过发现广播发送，也不保存到磁盘。更改 PIN 后，在 Windows 控制中心重新应用 PIN，并在 Android 端输入新 PIN。
